...
"""Input pipeline for text classification datasets using pickle files."""
...
def get_tc_datasets(n_devices,
                    ...
                    max_length=512,
                    tokenizer='char'):
  """Get text classification datasets from pickle files."""
  ...
  # Map task names to pickle file prefixes
  task_to_prefix = {
      'imdb_reviews': 'lra-text',
      'yelp_reviews': 'lra-text',  # May need adjustment
      'agnews': 'lra-text',  # May need adjustment
  }
  
  file_prefix = task_to_prefix.get(task_name, 'lra-text')
